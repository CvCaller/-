...
void HOG_SVM_detect::generate_dataset()
{
	...
	glob("D:/源码与资料图像/elec_watch/positive", postive_image);
	...
	glob("D:/源码与资料图像/elec_watch/negative", negative_image);
...
void HOG_SVM_detect::svm_train()
{
	...
	svm->save("D:/源码与资料图像/elec_watch/self_hog_elec.xml");
}
void HOG_SVM_detect::detecting(Mat frame)
{
	//Mat image = frame.clone();

	Ptr<SVM> svm1 = SVM::load("D:/源码与资料图像/elec_watch/self_hog_elec.xml");
...
Mat QRcodedetect::transform(Mat& image)
{
	...
	QRcodeCornerdetect(QRcodeCorner, contours_pts);//过滤误判轮廓
		...
		imwrite("D:/源码与资料图像/test.png", result3);
		imwrite("D:/源码与资料图像/test1.png", image);
